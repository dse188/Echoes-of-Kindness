...
public class M_Jump : M_IState
{
    public void StateEnter(M_StateMachine player)
    {
        Debug.Log("I'm in Jump State!");
    }
    ...
    {
        if(player.Player_OnGround() && player.Player_MovementValue() <= 0.01)
        {
            player.M_SwitchState(player.idleState);
        }
        else if (player.Player_OnGround() && player.Player_MovementValue() > 0.01)
        {
